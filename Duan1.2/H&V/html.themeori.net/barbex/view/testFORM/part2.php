...
function build_calendar($month, $year){
    ...
    $monthName = $dateComponents['month']; // Define the month name
    $dayOfWeek = $dateComponents['wday'];
    $datetoday = date('Y-m-d');

    $calendar = "<center><h2>$monthName $year</h2></center>"; // Initialize the $calendar variable
    ...
    $calendar .= "<a class='btn btn-primary btn-xs' href='?month=".date('m',mktime(0,0,0, $month+1,1,$year))."&year=".date('Y',mktime(0,0,0, $month+1,1,$year))."'>Next Month</a>";
    ...
    while($currentDay <= $numberDays){
        ...
        if ($date < $datetoday) {
            ...
            $calendar .= "<td class='$today'><h4>$currentDay</h4><button class='btn btn-success btn-xs'>Book</button></td>";
        }
